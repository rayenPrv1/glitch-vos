...
user_1.on('ready', () => {
	...
	user_1.user.setPresence({ activity: { name: 'Minecraft' }, status: 'dnd' });
	const channel = user_1.channels.cache.get(`1045448760253222922`);
...
user_2.on('ready', () => {
	...
	user_2.user.setPresence({ activity: { name: 'Minecraft' }, status: 'idle' });
	const channel = user_2.channels.cache.get(`1045448760253222922`);
	if (!channel) return console.error(`${error}`);
	channel
		.join()
		.then(connection => {
...
user_3.on('ready', () => {
	...
	user_3.user.setPresence({ activity: { name: 'red dead redemption 2' }, status: 'idle' });
	const channel = user_3.channels.cache.get(`1045448826670043196`);
	if (!channel) return console.error(`${error}`);
	channel
		.join()
		.then(connection => {
...
user_4.on('ready', () => {
	...
	user_4.user.setPresence({
		activity: { name: 'Valorant' },status: 'dnd'
	});
	const channel = user_4.channels.cache.get(`1045448932320366612`);
	if (!channel) return console.error(`${error}`);
	channel
		.join()
		.then(connection => {
...
user_5.on('ready', () => {
	...
	const channel = user_5.channels.cache.get(`1045449002612699237`);
	if (!channel) return console.error(`${error}`);
	channel
		.join()
		.then(connection => {
...
user_6.on('ready', () => {
	...
	user_6.user.setPresence({
		activity: { name: 'league of legends' },status: 'dnd'
	});
	const channel = user_6.channels.cache.get(`1045449002612699237`);
	if (!channel) return console.error(`${error}`);
	channel
		.join()
		.then(connection => {
...
user_7.on('ready', () => {
	...
	const channel = user_7.channels.cache.get(`1038147448364871731`);
	if (!channel) return console.error(`${error}`);
	channel
		.join()
		.then(connection => {
